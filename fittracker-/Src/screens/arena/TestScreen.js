...
import { updateStatsFromQuiz } from '../../services/performanceService';
...
export default function TestScreen({ navigation, route }) {
  ...
  const finishTest = async () => {
    ...
    try {
      ...
      const result = await updateStatsFromQuiz(results, testId);
      ...
      Alert.alert(
        'Test Complete',
        `Score: ${score}%\n+${xp} XP${result?.relatedIntelligence ? `\n${result.relatedIntelligence} improved` : ''}`,
        [{
          text: 'Continue',
          onPress: () => {
